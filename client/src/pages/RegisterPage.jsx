...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      saveAuth(token, user);
      showToast(`Акаунт створено! Ласкаво просимо, ${user.name}!`);
      navigate('/');
    } catch (err) {
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <label className={styles.label}>Ім'я
          <input className={styles.input} type="text" value={name}
            onChange={e => setName(e.target.value)} required placeholder="Ваше ім'я" />
        </label>
        ...
        <label className={styles.label}>Пароль
          <input className={styles.input} type="password" value={password}
            onChange={e => setPassword(e.target.value)} required placeholder="мін. 6 символів" />
        </label>
        <button className={styles.button} type="submit" disabled={loading}>
          {loading ? 'Реєстрація...' : 'Зареєструватись'}
        </button>
        <p className={styles.link}>Вже є акаунт? <Link to="/login">Увійти</Link></p>
      </form>
    </div>
  );
}